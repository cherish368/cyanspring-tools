...
public class PriceUtil {
    ...
    static public int CompareBySide(boolean buy, double x, double y) {
        if (buy)
            return PriceUtil.Compare(y, x);
        else
            return PriceUtil.Compare(x, y);
    }
    ...
    static public boolean sameSide(double x, double y) {
        return PriceUtil.GreaterThan(x, 0) && PriceUtil.GreaterThan(y, 0) ||
                PriceUtil.LessThan(x, 0) && PriceUtil.LessThan(y, 0) ||
                PriceUtil.isZero(x) && PriceUtil.isZero(y);
    }

    static public boolean canBeDivided(double x, double y) {
        double mode = x % y;
        return PriceUtil.isZero(mode) || PriceUtil.Equal(mode, y);
    }
}
